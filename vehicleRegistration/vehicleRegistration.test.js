...
describe('Vehicle Registration API', () => {
  ...
  it('should fetch vehicles by owner ID', async () => {
    const ownerId = 1; // Replace with a valid owner ID for testing
  ...
  it('should create a new vehicle', async () => {
    const newVehicle = {
      ...
      color: 'Red',
      tipo: 'Sedan',
      propietario_id: 1, // Replace with a valid owner ID
      status: 'active',
    };
  ...
  it('should update a vehicle by ID', async () => {
    const vehicleId = 1; // Replace with a valid vehicle ID for testing
    ...
    const response = await request(app).put(`/vehicles/${vehicleId}`).send(updatedVehicle);
    if (response.status === 200) {
  ...
  it('should delete a vehicle by ID', async () => {
    const vehicleId = 1; // Replace with a valid vehicle ID for testing
    const response = await request(app).delete(`/vehicle/${vehicleId}`);
    if (response.status === 200) {
      expect(response.body.message).toBe('Vehiculo eliminado correctamente');
    } else {
      expect(response.status).toBe(404);
      expect(response.body.message).toBe('Vehiculo no encontrada');
    }
  });
});